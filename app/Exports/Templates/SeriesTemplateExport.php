...
use Maatwebsite\Excel\Concerns\{FromArray, WithHeadings, WithStyles, WithTitle, WithColumnWidths, WithMultipleSheets};
use PhpOffice\PhpSpreadsheet\Worksheet\Worksheet;

class SeriesTemplateExport implements WithMultipleSheets {
    public function sheets(): array { return [new SeriesDataSheet(), new SeriesProductRefSheet()]; }
}

class SeriesDataSheet implements FromArray, WithHeadings, WithStyles, WithTitle, WithColumnWidths {
    public function array(): array {
        $rows = Series::with('product')->orderBy('code')->get()->map(fn($s) => [
            $s->code, $s->name, $s->product?->code ?? '',
        ])->toArray();
        return $rows ?: [
    ...
    public function headings(): array { return ['kode','nama','kode_produk']; }
    public function title(): string { return 'Data Series'; }
    public function columnWidths(): array { return ['A'=>18,'B'=>30,'C'=>15]; }
...
class SeriesProductRefSheet implements FromArray, WithHeadings, WithStyles, WithTitle, WithColumnWidths {
    ...
    public function headings(): array { return ['kode_produk','nama_produk','kategori']; }
    public function title(): string { return 'Ref Produk'; }
    public function columnWidths(): array { return ['A'=>15,'B'=>30,'C'=>20]; }
    public function styles(Worksheet $sheet): array {
        return [1=>['font'=>['bold'=>true,'color'=>['rgb'=>'FFFFFF']],'fill'=>['fillType'=>'solid','startColor'=>['rgb'=>'70AD47']]]];
    }
}

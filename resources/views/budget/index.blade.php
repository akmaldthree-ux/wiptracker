...
<div class="row g-3 mb-4">
  <div class="col-12 col-12 col-md-4"><div class="kpi-card kpi-blue"><div class="kpi-icon"><i class="bi bi-wallet2"></i></div><div class="kpi-value">Rp {{ number_format($totalPlan/1000000,1) }}jt</div><div class="kpi-label">Total Budget Rencana</div></div></div>
  <div class="col-12 col-md-4"><div class="kpi-card {{ $totalActual>$totalPlan?'kpi-red':'kpi-teal' }}"><div class="kpi-icon"><i class="bi bi-cash-coin"></i></div><div class="kpi-value">Rp {{ number_format($totalActual/1000000,1) }}jt</div><div class="kpi-label">Total Realisasi</div></div></div>
  <div class="col-12 col-md-4"><div class="kpi-card {{ $totalActual>$totalPlan?'kpi-red':'kpi-green' }}"><div class="kpi-icon"><i class="bi bi-graph-up"></i></div><div class="kpi-value">{{ $totalPlan>0?number_format(($totalActual/$totalPlan)*100,1):0 }}%</div><div class="kpi-label">Utilisasi Budget</div></div></div>
</div>
<div class="card">
  <div class="card-header"><i class="bi bi-wallet2 me-2 text-primary"></i>Budget per Order Produksi</div>
  <div class="table-responsive">
    <table class="table table-hover mb-0">
      <thead><tr><th>Order</th><th>Produk</th><th class="d-mob-none">Budget Plan</th><th class="d-mob-none">Realisasi</th><th class="d-mob-none">Variance</th><th>Utilisasi</th><th>Status</th><th></th></tr></thead>
      <tbody>
        @forelse($orders as $o)
        ...
        <tr class="{{ $isOver ? 'table-danger' : '' }}">
          ...
          <td class="d-mob-none">Rp {{ $b ? number_format($b->total_plan) : '-' }}</td>
          <td class="d-mob-none">Rp {{ $b ? number_format($b->total_actual) : '-' }}</td>
          <td class="d-mob-none {{ $isOver?'text-danger fw-bold':'text-success fw-bold' }}">{{ $b ? 'Rp '.number_format($b->variance) : '-' }}</td>
          <td>
            @if($b)

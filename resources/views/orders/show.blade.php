...
<div class="d-flex justify-content-between align-items-start mb-4">
  ...
  <div class="d-flex gap-2">
    ...
    @if($order->status === 'active' && in_array(auth()->user()->role,['admin','supervisor']))
      @php $sentToCutting = $order->handovers->whereNull('from_station_id')->whereIn('status',['pending','confirmed','approved'])->isNotEmpty(); @endphp
      @if(!$sentToCutting)
      <form method="POST" action="{{ route('orders.send-to-cutting',$order) }}" class="d-inline" onsubmit="return confirm('Kirim order ini ke Cutting?')">
        @csrf
        <button type="submit" class="btn btn-success"><i class="bi bi-scissors me-1"></i>Kirim ke Cutting</button>
      </form>
      @else
      <button class="btn btn-success disabled"><i class="bi bi-check-circle me-1"></i>Sudah Dikirim ke Cutting</button>
      @endif
    @endif
    ...
    <div class="card">
      ...
      <div class="list-group list-group-flush">
        ...
        <a href="{{ route('handover.show',$h) }}" class="list-group-item list-group-item-action py-3">
          ...
          <small class="text-muted">{{ $h->fromStation?->name ?? 'Production Order' }} → {{ $h->toStation->name }}</small>

...
.orb-red {
  ...
  background: radial-gradient(circle at 40% 40%, rgba(234,34,97,.65) 0%, transparent 70%);
  top: -180px; left: -120px;
  animation: orbRed 20s ease-in-out infinite;
}
.orb-blue {
  width: 500px; height: 500px;
  background: radial-gradient(circle at 55% 45%, rgba(83,58,253,.6) 0%, transparent 70%);
  bottom: -160px; right: -100px;
  animation: orbBlue 26s ease-in-out infinite;
}
.orb-purple {
  width: 420px; height: 420px;
  background: radial-gradient(circle at 45% 55%, rgba(102,94,253,.55) 0%, transparent 70%);
  top: 38%; left: 42%;
  transform: translate(-50%, -50%);
  animation: orbPurple 18s ease-in-out infinite;
}
@keyframes orbRed {
  0%,100% { transform: translate(0,0)      scale(1);    }
  25%      { transform: translate(60px,-40px) scale(1.08); }
  50%      { transform: translate(30px,70px)  scale(.95);  }
  75%      { transform: translate(-40px,20px) scale(1.05); }
}
@keyframes orbBlue {
  0%,100% { transform: translate(0,0)       scale(1);    }
  30%      { transform: translate(-70px,30px)  scale(1.1);  }
  60%      { transform: translate(40px,-60px)  scale(.93);  }
  80%      { transform: translate(-20px,50px)  scale(1.06); }
}
@keyframes orbPurple {
  0%,100% { transform: translate(-50%,-50%) scale(1);    }
  35%      { transform: translate(-42%,-58%) scale(1.12); }
  70%      { transform: translate(-58%,-44%) scale(.92);  }
}
...
.grid-layer {
  ...
  background-image:
    linear-gradient(rgba(83,58,253,.07) 1px, transparent 1px),
    linear-gradient(90deg, rgba(83,58,253,.07) 1px, transparent 1px);
...
.vignette {
  ...
  background: radial-gradient(ellipse 70% 70% at 50% 50%, transparent 35%, rgba(13,13,13,.75) 100%);
}
...
.card-left {
  ...
  background: #0c0c14;
  background-image:
    radial-gradient(ellipse 90% 60% at 0% 0%,   rgba(234,34,97,.18) 0%,  transparent 55%),
    radial-gradient(ellipse 60% 50% at 100% 100%, rgba(83,58,253,.2)  0%,  transparent 55%);
  padding: 2.5rem 2rem;
...
.card-left::before {
  ...
  background: linear-gradient(90deg, transparent, var(--ruby) 30%, var(--primary-soft) 70%, transparent);
}
/* Shine sweep — every 6s */
.card-left::after {
  content: '';
...
.system-badge {
  ...
  background: rgba(234,34,97,.12);
  border: 1px solid rgba(234,34,97,.28);
  border-radius: 9999px; padding: .28rem .75rem;
  font-size: .65rem; font-weight: 500;
  color: rgba(234,34,97,.85); letter-spacing: 1px; text-transform: uppercase;
  margin-bottom: 1.6rem; align-self: flex-start;
}
.system-badge .dot {
  width: 5px; height: 5px; background: var(--ruby); border-radius: 50%;
...
.brand-icon-wrap {
  position: relative; width: 52px; height: 52px; margin-bottom: 1.1rem;
}
.brand-icon-wrap img {
...
.brand-icon-wrap::before {
  ...
  background: radial-gradient(circle, rgba(234,34,97,.55) 0%, transparent 70%);
...
.brand-name {
  ...
  letter-spacing: -1.3px; line-height: 1.1; margin-bottom: .25rem;
  font-feature-settings: "ss01";
  background: linear-gradient(120deg, var(--ruby) 0%, #f97fb5 55%, var(--primary-subdued) 100%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent;
  background-clip: text;
}
.brand-tagline { font-size: .72rem; color: rgba(255,255,255,.28); letter-spacing: .3px; margin-bottom: 1.7rem; }

/* Feature items — stagger enter */
.features { display: flex; flex-direction: column; gap: .5rem; flex: 1; }
.feature-item {
...
.feature-item:hover {
  background: rgba(234,34,97,.08);
  border-color: rgba(234,34,97,.2);
  transform: translateX(3px);
}
.feature-icon {
  width: 30px; height: 30px; flex-shrink: 0; border-radius: 8px;
  background: rgba(234,34,97,.14);
  border: 1px solid rgba(234,34,97,.22);
  display: flex; align-items: center; justify-content: center;
  font-size: .8rem; color: rgba(234,34,97,.85);
}
.feature-text .ft { font-size: .77rem; font-weight: 500; color: rgba(255,255,255,.82); }
.feature-text .fd { font-size: .66rem; color: rgba(255,255,255,.28); margin-top: 1px; }

.left-footer {
  margin-top: 1.5rem;
  display: flex; justify-content: space-between; align-items: center;
}
.version-tag { font-size: .62rem; color: rgba(255,255,255,.16); }
.status-tag  { display: flex; align-items: center; gap: .3rem; font-size: .62rem; color: rgba(234,34,97,.55); }
.status-tag .sdot { width: 4px; height: 4px; background: var(--ruby); border-radius: 50%; animation: blink 2s ease-in-out infinite; }
...
.card-right::before {
  content: '';
  position: absolute; top: 0; left: 0; right: 0; height: 2px;
  background: linear-gradient(90deg, var(--ruby) 0%, var(--primary-soft) 50%, var(--ruby) 100%);
...
.form-heading h2 {
  ...
  letter-spacing: -.6px; font-feature-settings: "ss01";
}
...
.field label {
  ...
  color: var(--ink-mute); letter-spacing: .4px; text-transform: uppercase; margin-bottom: .35rem;
}
...
.input-wrap input {
  width: 100%; padding: .7rem .8rem .7rem 2.35rem;
  border: 1px solid var(--hairline-input); border-radius: 6px;
...
.pw-toggle {
  ...
  color: var(--hairline-input); font-size: .85rem; transition: color .13s; padding: 2px; line-height: 1;
}
.pw-toggle:hover { color: var(--primary); }

.remember-row {
  display: flex; align-items: center; gap: .5rem; margin: 1rem 0 1.2rem;
}
.remember-row input[type="checkbox"] { width: 14px; height: 14px; accent-color: var(--primary); cursor: pointer; }
...
.btn-login:hover  { background: var(--primary-deep); box-shadow: 0 6px 22px rgba(83,58,253,.46); transform: translateY(-1px); }
...
@media (max-width: 680px) {
  ...
  .features, .left-footer { display: none; }
...
<div class="card">
  ...
  <div class="card-left">
    <div class="left-inner">
      ...
      <div class="system-badge"><span class="dot"></span>Production System</div>

      <div class="brand-icon-wrap">
        <img src="{{ asset('images/dthree-logo.png') }}" alt="DTHREE">
      </div>
      <div class="brand-name">DPIS<br>Dthree</div>
  ...
  <div class="card-right">
    ...
    <div class="demo-grid">
      <button class="demo-btn indigo" onclick="fillDemo('admin@dthree.id')">
        <span class="demo-email">admin@dthree.id</span><span class="demo-role">Admin</span>
      </button>
      <button class="demo-btn navy" onclick="fillDemo('supervisor@dthree.id')">
        <span class="demo-email">supervisor@dthree.id</span><span class="demo-role">Supervisor</span>
      </button>
      <button class="demo-btn slate" onclick="fillDemo('manager@dthree.id')">
        <span class="demo-email">manager@dthree.id</span><span class="demo-role">Manager</span>
      </button>
      <button class="demo-btn rose" onclick="fillDemo('cutting@dthree.id')">
        <span class="demo-email">cutting@dthree.id</span><span class="demo-role">Staff Cutting</span>
      </button>
    </div>
...
(function() {
  ...
  const colors = [
    'rgba(234,34,97,.75)', 'rgba(234,34,97,.5)',
    'rgba(83,58,253,.7)',  'rgba(83,58,253,.5)',
    'rgba(102,94,253,.65)','rgba(185,185,249,.5)',
    'rgba(249,107,238,.5)','rgba(255,255,255,.25)',
  ];
  for (let i = 0; i < 28; i++) {
    const p   = document.createElement('div');
    p.className = 'particle';
    const sz  = Math.random() * 2.8 + 1;
    ...
    p.style.cssText = `
      ...
      --dx: ${dx}px;
      border-radius: 50%;
      filter: blur(${Math.random() > .55 ? .8 : 0}px);
    `;
    cont.appendChild(p);
  }
})();
